...
/**
 * Tạo người dùng mới (Admin only - dùng API POST /api/users thay vì signup)
 * @param {Object} userData - Dữ liệu người dùng
 * @returns {Promise<Object>} Kết quả tạo người dùng
 */
export const createUser = async (userData) => {
  try {
    console.log("📝 [User] Đang tạo người dùng mới:", userData);

    const response = await api.post("/api/users", userData);

    console.log("✅ [User] Tạo thành công:", response.data);
    ...
    console.error("❌ [User] Lỗi tạo người dùng:", error);
    ...
    return {
      success: false,
      message: error.message || "Không thể tạo người dùng. Vui lòng thử lại.",
    };
  }
};
